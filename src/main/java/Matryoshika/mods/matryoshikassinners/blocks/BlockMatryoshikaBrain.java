...
public class BlockMatryoshikaBrain extends BlockContainer {

	private Random rand = new Random();

	public BlockMatryoshikaBrain(Block matryoshikaBrain) {
		super(Material.rock);
		setStepSound(soundTypePiston);
		setBlockTextureName(matryoshikassinners.MODID+":matryoshikaBrainTEMP");
		setHardness(15);
		setResistance(1000000);
	    this.setBlockName("BlockMatryoshikaBrain");
	}
	...
	public void breakBlock(World w, int x, int y, int z, Block b, int i) {
		TileEntityMatryoshikaBrain items = (TileEntityMatryoshikaBrain)w.getTileEntity(x, y, z);
		if(items != null) {
	...
	private IInventory getInventory(World w, int x, int y, int z) {
		Object o = (TileEntityMatryoshikaBrain)w.getTileEntity(x, y, z);
		return (IInventory)o;
	}
	...
	public TileEntity createNewTileEntity(World var1, int var2) {
		return new TileEntityMatryoshikaBrain();
	}
}
